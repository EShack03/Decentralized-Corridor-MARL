...
class GMPERunner(Runner):
	...
	def get_agent_states(self, envs):
		states = {}
		agents = envs.envs[0].world.agents

		for i, agent in enumerate(agents):
			states[i] = agent.state.values.copy()
	...
	def render(self, get_metrics:bool=False):
		...
		for episode in range(self.all_args.render_episodes):
			...
			for step in range(self.episode_length):
				...
				# # --- BlueSky live visualization ---
				# agent_states = self.get_agent_states(envs)
